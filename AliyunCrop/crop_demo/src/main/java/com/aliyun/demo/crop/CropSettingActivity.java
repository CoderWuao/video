    ...
    public void onClick(View v) {
        if(v == startImport){
            ...
            if(TextUtils.isEmpty(g)){
                ...
                long inputGop = Long.parseLong(g);
                if (inputGop > Integer.MAX_VALUE) {
                    gop = Integer.MAX_VALUE;
                } else {
                    gop = Integer.parseInt(g);
                }
            }
            ...
            if(TextUtils.isEmpty(g)){
                ...
                long inputRate = Long.valueOf(f);
                if(inputRate > Integer.MAX_VALUE){
                    frameRate = DEFAULT_FRAMR_RATE;
                }else{
                    frameRate = Integer.parseInt(f);
                }
            }

            int bitrate = 0;
            String s = mBitrateEdit.getText().toString();
            if(!TextUtils.isEmpty(s)){
                long inputBitrate = Long.parseLong(s);
                if (inputBitrate > Integer.MAX_VALUE) {
